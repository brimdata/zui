...
test("c packet with a bad checksum", () => {
  expect(connHistoryView("c")).toEqual([{ text: "bad chksum", direction: "left" }]);
});
...
test("i inconsistent packet (e.g. FIN+RST bits set)", () => {
  expect(connHistoryView("i")).toEqual([{ text: "inconsistent", direction: "left" }]);
});
...
test("complex history", () => {
  expect(connHistoryView("ShADd^qITc")).toEqual([{ text: "syn", direction: "right" }, { text: "syn ack", direction: "left" }, { text: "ack", direction: "right" }, { text: "data", direction: "right" }, { text: "data", direction: "left" }, { text: "multi", direction: "left" }, { text: "inconsistent", direction: "right" }, { text: "retrans", direction: "right" }, { text: "bad chksum", direction: "left" }]);
});