...
describe("parseInZone", () => {
  ...
  test("strict date in all zones", () => {
    names.forEach(zone => {
      ...
      let str = lib.date(brim.time(ts || new Date()).toDate()).zone(zone).format(fmt);
  ...
  test("reference date in all zones", () => {
    names.forEach(zone => {
      ...
      let str = lib.date(brim.time(date || new Date()).toDate()).zone(zone).format(fmt);
  ...
  test("casual date in all zones", () => {
    names.forEach(zone => {
      ...
      let str = brim.time(date || new Date()).toDate().toISOString();
