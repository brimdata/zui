...
function zeekRunnerCommand(zeekRunnerPref: string): string {
  ...
  const precedence = [process.env.BRIM_ZEEK_RUNNER, zeekRunnerPref, resolve(join(zdepsDirectory, "zeek", plat.zeekRunnerBin))];
...
export class ZQD {
  ...
  start() {
    ...
    let args = ["listen", "-l", this.addr(), "-data", this.root, "-config", confFile, "-zeekrunner", zeekRunnerCommand(this.zeekRunner)];
