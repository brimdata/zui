...
const records = [[{ name: "_td", type: "string", value: "1" }, { name: "ts", type: "time", value: "100" }], [{ name: "_td", type: "string", value: "1" }, { name: "ts", type: "time", value: "200" }], [{ name: "_td", type: "string", value: "1" }, { name: "ts", type: "time", value: "300" }]];
...
beforeEach(() => {
  ...
  store.dispatchAll([Clusters.add(conn), Spaces.setDetail(conn.id, space), Current.setConnectionId(conn.id), Current.setSpaceId(space.id), Search.setSpanArgsFromDates([new Date(0), new Date(10 * 1000)]), Tab.computeSpan(), Viewer.appendRecords(tabId, records)]);
  store.clearActions();
});
...
test("#fetchNextPage dispatches splice", () => {
  ...
  expect(store.getActions()).toEqual(expect.arrayContaining([{ tabId, type: "VIEWER_SPLICE", index: 2 }]));
});
...
test("#fetchNextPage adds 1ms to ts of last change", () => {
  ...
  expect(search).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
    from: new Date(0),
