...
function menuText(menu: MenuItem) {
  return menu.filter(item => item.enabled).map(item => item.label).join(", ");
}
...
describe("Log Right Click", () => {
  ...
  test("conn log with pcap support", () => {
    ...
    const ctxMenu = menu.searchFieldContextMenu(program, log.descriptor.map(c => c.name), space)(field, log, false);
...
describe("Analysis Right Click", () => {
  ...
  test("non-address field", () => {
    ...
    const ctxMenu = menu.searchFieldContextMenu("* | count() by proto", log.descriptor.map(c => c.name), space)(field, log, false);
  ...
  test("group by proc", () => {
    const log = conn();
    const field = log.mustGetField("proto");
    const ctxMenu = menu.searchFieldContextMenu("* | group by proto", log.descriptor.map(c => c.name), space)(field, log, false);
