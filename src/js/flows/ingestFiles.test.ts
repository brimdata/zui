...
beforeEach(() => {
  zealot = createZealotMock().stubStream("pcaps.post", [{ type: "TaskStart" }, {
    type: "PcapPostStatus",
    snapshot_count: 1,
    ...
    pcap_total_size: 100,
    pcap_read_size: 1
  }, { type: "TaskEnd" }]).stubPromise("spaces.create", {
...
test("opening a pcap", async () => {
  ...
  expect(Current.mustGetSpace(state)).toEqual(expect.objectContaining({
    name: "sample.pcap.brim",
    id: "spaceId",
    min_time: { ns: 0, sec: 0 },
    max_time: { ns: 1, sec: 1 },
...
test("when there is an error", async () => {
  ...
  await expect(store.dispatch(ingestFiles([itestFile("sample.pcap")], globalDispatch))).rejects.toEqual(expect.any(Error));
...
test("a zeek ingest error", async () => {
  ...
  await expect(store.dispatch(ingestFiles([itestFile("sample.tsv")], globalDispatch))).rejects.toEqual(expect.any(Error));
...
test("a json file with a custom types config", async () => {
  ...
  await store.dispatch(ingestFiles([itestFile("sample.ndjson")], globalDispatch));
