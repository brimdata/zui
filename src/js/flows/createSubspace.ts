...
export default ((): Thunk => (dispatch, getState) => {
  ...
  if (!records.length) return Promise.reject(new CreateSubspaceError("No selected logs"));
  ...
  try {
    ...
    return zealot.subspaces.create({ name, spaceId, logs }).then(space => dispatch(refreshSpaceNames()).then(() => dispatch(Tabs.new(space.id))));
  } catch (e) {
