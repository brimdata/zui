...
beforeEach(() => {
  ...
  store.dispatchAll([Current.setConnectionId("1"), Spaces.setDetail("1", space), Current.setSpaceId(space.id), SearchBar.changeSearchBarInput(":ip=192.168.0.1"), SearchBar.setTarget("index")]);
});
...
describe("table search", () => {
  test("zealot sends one request", async () => {
    ...
    expect(calls[0].args).toEqual(expect.objectContaining({ patterns: [":ip=192.168.0.1"], spaceId: "1" }));
  });
  ...
  test("the table gets cleared", async () => {
    dispatch(Viewer.setRecords(undefined, [[{ name: "clear", type: "string", value: "me" }]]));
    submit();
  ...
  test("registers a table request then cleans it up", async () => {
    const promise = submit();
    expect(select(Handlers.get)["Table"]).toEqual(expect.objectContaining({ type: "SEARCH" }));
    await promise;
  ...
  test("search warnings", async () => {
    ...
    await submit();
    expect(select(SearchBar.getSearchBarError)).toBe("Cut field boo not present in input");
  });
});