...
describe("table search", () => {
  ...
  test("the table gets cleared", async () => {
    ...
    await submit();
    expect(select(Viewer.getViewerRecords)[0]).not.toEqual([{ name: "clear", type: "string", value: "me" }]);
  });
  ...
  test("registers historgram request then cleans it up", async () => {
    zealot.stubStream("search", countByPathResp);
    const promise = submit();
    expect(select(Handlers.get)["Histogram"]).toEqual(expect.objectContaining({ type: "SEARCH" }));
    await promise;
