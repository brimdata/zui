...
describe("analytic search", () => {
  ...
  test("zealot gets the right args", async () => {
    ...
    expect(zealot.calls("search")[0].args).toEqual("dns _path=conn | count() | head 10000");
  });

  test("the table gets populated", async () => {
    await submit();
    expect(select(Viewer.getViewerRecords)).toEqual([[{ name: "count", type: "count", value: "8100" }]]);
  });
