...
export function submitSearch(save?: SaveOpts = { history: true, investigation: true }, ts: Date = new Date()): Thunk {
  return function (dispatch, getState) {
    ...
    if (record.target === "index") {
      ...
      const promise = dispatch(viewerSearch({ query: tableProgram, from, to, target: "events" }));
      dispatch(Last.setSearch(record));
      return promise;
    }
  };
