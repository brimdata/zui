...
export default ((paths: string[], gDispatch: Dispatch = globalDispatch): Thunk => (dispatch, getState) => {
  ...
  return lib.transaction([validateInput(paths, dataDir, spaceNames), createDir(), createSpace(zealot, gDispatch, clusterId), registerHandler(dispatch, requestId), postFiles(zealot, jsonTypeConfigPath), setSpace(dispatch, tabId), trackProgress(zealot, gDispatch, clusterId), unregisterHandler(dispatch, requestId)]);
});

const validateInput = (paths, dataDir, spaceNames) => ({
  async do() {
    let params = await ingest.detectFileTypes(paths).then(data => ingest.getParams(data, dataDir, spaceNames)).catch(e => {
      if (e.message.startsWith("EISDIR")) throw new Error("Importing directories is not yet supported. Select multiple files.");else throw e;
    });
...
const createSpace = (client, gDispatch, clusterId) => ({
  async do(params) {
    ...
    gDispatch(Spaces.setDetail(clusterId, {
      ...space,
...
const postFiles = (client, jsonTypesPath) => ({
  async do(params) {
    ...
    if (endpoint === "pcap") {
      ...
      let types = isEmpty(jsonTypesPath) ? undefined : await lib.file(jsonTypesPath).read().then(JSON.parse);
  ...
  return {
    async do({
      ...
      function packetPostStatusToPercent(status): number {
        if (status.pcap_total_size === 0) return 1;else return status.pcap_read_size / status.pcap_total_size || 0;
      }

      function logPostStatusToPercent(status): number {
        // log_total_size may not be present
        if (!status.log_total_size) return 1;else return status.log_read_size / status.log_total_size;
      }
      ...
      for await (let {
        ...
        switch (type) {
          case "PcapPostStatus":
            gDispatch(space.setIngestProgress(packetPostStatusToPercent(status)));
