...
export function viewerSearch(args: Args): Thunk {
  return (dispatch, getState) => {
    ...
    const {
      response,
      promise
    } = dispatch(search({ id, query, from, to, spaceId, target }));
    dispatch(handle(response, tabId, isBlocking, append));
    return promise;
  };
}

function handle(response: any, tabId: string, isBlocking?: boolean = false, append?: boolean = false): Thunk {
  return function (dispatch) {
    ...
    response.status(status => dispatch(Viewer.setStatus(tabId, status))).chan(0, (records, types) => {
    ...
    }).warnings(warning => dispatch(SearchBar.errorSearchBarParse(warning))).error(error => {
