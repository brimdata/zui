...
export default class TableColumns {
  ...
  constructor(id: string, columns: $Column[] = [], tableSetting: ColumnSettingsMap = {}) {
    this.id = id;
    this.cols = columnOrder(columns).map(({
      name,
      type,
      key
    }, index) => ({
  ...
  sumWidths() {
    return this.getVisible().reduce((sum, column) => sum += column.width || 0, 0);
  }
