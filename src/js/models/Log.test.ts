...
test("isSame when tuples are the same", () => {
  const a = new Log(["1", "conn"], [{ type: "integer", name: "td" }, { type: "string", name: "_path" }]);
  const b = new Log(["1", "conn"], [{ type: "integer", name: "td" }, { type: "string", name: "_path" }]);
  expect(Log.isSame(a, b)).toBe(true);
});

test("isSame when tuples are different", () => {
  const a = new Log(["1", "conn"], [{ type: "integer", name: "td" }, { type: "string", name: "_path" }]);
  const b = new Log(["2", "conn"], [{ type: "integer", name: "td" }, { type: "string", name: "_path" }]);
...
describe(".sort", () => {
  ...
  const getTs = t => t.getString("ts");

  test("asc", () => {
    Log.sort(logs, "ts", "asc");

    expect(logs.map(getTs)).toEqual(["1425565514.419939", "1425567042.047800", "1425612054.369843"]);
  });

  test("desc", () => {
    Log.sort(logs, "ts", "desc");

    expect(logs.map(getTs)).toEqual(["1425612054.369843", "1425567042.047800", "1425565514.419939"]);
  });
});
