...
export default function (p: string = "", pins: string[] = []) {
  ...
  return {
    ...
    cut(...fields: string[]) {
      p = stdlib.string(p).append(" | cut " + fields.join(", ")).self();
      return this;
    },

    drillDown(log: $Log) {
      let filter = this.filter();
      let newFilters = this.ast().groupByKeys().map(n => log.field(n)).filter(f => !!f) // $FlowFixMe flow doesn't know I just took out all the nils
      .map(brim.syntax.include).join(" ");
    ...
    countBy(field: $Field) {
      p = stdlib.string(p).append(" | " + brim.syntax.countBy(field)).trim().self();

      return this;
    },

    sortBy(name: string, direction: "asc" | "desc" = "asc") {
      p = stdlib.string(p).replace(/\|\s*sort[^|]*$/i, "").append(" | " + brim.syntax.sortBy(name, direction)).trim().self();
...
function insertFilter(program, filter) {
  return stdlib.string(program).insert(" " + filter + " ", filterEnd(program)).trim().self();
}
