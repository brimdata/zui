...
test("convert to count", () => {
  expect(zngToZeekTypes([{name: "one", type: "uint64"}])).toEqual([
    {name: "one", type: "count"}
  ])
})

test("convert to double", () => {
  expect(zngToZeekTypes([{name: "one", type: "float64"}])).toEqual([
    {name: "one", type: "double"}
  ])
})

test("convert to addr", () => {
  expect(zngToZeekTypes([{name: "one", type: "ip"}])).toEqual([
    {name: "one", type: "addr"}
  ])
})

test("convert to subnet", () => {
  expect(zngToZeekTypes([{name: "one", type: "net"}])).toEqual([
    {name: "one", type: "subnet"}
  ])
})

test("convert to interval", () => {
  expect(zngToZeekTypes([{name: "one", type: "duration"}])).toEqual([
    {name: "one", type: "interval"}
  ])
})

test("convert to string", () => {
  expect(zngToZeekTypes([{name: "one", type: "bstring"}])).toEqual([
    {name: "one", type: "string"}
  ])
})

test("convert to enum", () => {
  expect(zngToZeekTypes([{name: "one", type: "zenum"}])).toEqual([
    {name: "one", type: "enum"}
  ])
})

test("all other types passthrough", () => {
  expect(zngToZeekTypes([{name: "one", type: "port"}])).toEqual([
    {name: "one", type: "port"}
  ])
})
