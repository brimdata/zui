...
function zip(values, columns) {
  ...
  return values.map((value, index) => {
    ...
    // $FlowFixMe
    return isString(type) ? { name, type, value } : { name, type: "record", value: zip(value, type) };
  });
}

function flattenRecord(record, prefix): FieldData[] {
  // $FlowFixMe
  return record.reduce( // $FlowFixMe
...
function flatFields({
  ...
  // $FlowFixMe
  return type === "record" ? flattenRecord(value, `${prefix}${name}.`) : [{ name: prefix + name, type, value }];
}