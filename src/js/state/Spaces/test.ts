...
test("space names removing", () => {
  ...
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", testSpaces), Spaces.setSpaces("cluster1", [testSpace2])]);
...
test("setting the ingest progress", () => {
  ...
  store.dispatchAll([Spaces.setSpaces("cluster1", testSpaces), actions.setIngestProgress(0.5)]);

  let value = Spaces.getIngestProgress("cluster1", testSpace1.id)(store.getState());

  expect(value).toEqual(0.5);
});

test("getting the spaces with details, others not", () => {
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", testSpaces), Spaces.setDetail("cluster1", { ...detail })]);
  let spaces = Spaces.getSpaces("cluster1")(state);

  expect(spaces).toEqual([{ ...testSpace1, ingest: { warnings: [], progress: null, snapshot: null } }, { ...testSpace2, ingest: { warnings: [], progress: null, snapshot: null } }, {
    name: "defaultName",
    id: "defaultId",
...
test("ingest warnings", () => {
  let actions = Spaces.actionsFor("cluster1", testSpace1.id);
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", [testSpace1]), actions.appendIngestWarning("Problem 1"), actions.appendIngestWarning("Problem 2")]);

  expect(Spaces.getIngestWarnings("cluster1", testSpace1.id)(state)).toEqual(["Problem 1", "Problem 2"]);
});

test("clear warnings", () => {
  let actions = Spaces.actionsFor("cluster1", testSpace1.id);
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", [testSpace1]), actions.appendIngestWarning("Problem 1"), actions.appendIngestWarning("Problem 2"), actions.clearIngestWarnings()]);
...
test("remove space", () => {
  ...
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", [testSpace1]), actions.remove()]);
...
test("setting the spanshot counter", () => {
  ...
  let state = store.dispatchAll([Spaces.setSpaces("cluster1", [testSpace1]), actions.setIngestSnapshot(1)]);
...
test("rename a space", () => {
  ...
  const state = store.dispatchAll([Spaces.setSpaces("cluster1", [testSpace1]), Spaces.rename("cluster1", testSpace1.id, testRename)]);
