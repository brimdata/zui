...
const spacesReducer = produce((draft, action: SpacesAction) => {
  switch (action.type) {
    ...
    case "SPACES_DETAIL":
      var {
        id
      } = action.space; // XXX adapter hack to support span payloads from zqd as well as min/max
...
export default function reducer(state: SpacesState = init, action: SpacesAction): SpacesState {
...
function getSpace(state, id) {
  if (state[id]) return state[id];else throw new Error("No space exists with id: " + id);
}