...
beforeEach(() => {
  ...
  store.dispatchAll([Clusters.add(conn), Spaces.setDetail(conn.id, space), Current.setConnectionId(conn.id), Current.setSpaceId(space.id), SearchBar.changeSearchBarInput("first"), submitSearch(), SearchBar.changeSearchBarInput("second"), submitSearch(), SearchBar.changeSearchBarInput("third"), submitSearch()]);
});
...
test("going forward in history", () => {
  let state = store.dispatchAll([History.back(), History.back(), History.forward()]);
  const entry = Tab.currentEntry(state);
  expect(entry.program).toEqual("second");
});

test("going back in history then pushing new history", () => {
  let state = store.dispatchAll([History.back(), History.back(), SearchBar.changeSearchBarInput("fourth"), submitSearch()]);
  const entry = Tab.currentEntry(state);
  expect(entry.program).toEqual("fourth");
});

test("back, back, push, back", () => {
  let state = store.dispatchAll([History.back(), History.back(), SearchBar.changeSearchBarInput("fourth"), submitSearch(), History.back()]);
  const entry = Tab.currentEntry(state);
...
test("update scroll position, back, forward", () => {
  let state = store.dispatchAll([History.update({ x: 22, y: 33 }), History.back(), History.forward()]);
  const entry = Tab.currentEntry(state);
