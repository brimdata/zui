...
describe("Columns", () => {
  ...
  test("Bulk update column settings", () => {
    const state = store.dispatchAll([Columns.updateColumns(tableId, {
      "_path:string": {
        width: 22,
        isVisible: true,
        position: 0
  ...
  test("hide one column", () => {
    const state = store.dispatchAll([Columns.hideColumn(tableId, { name: "a", type: "string" })]);
  ...
  test("show one column", () => {
    const state = store.dispatchAll([Columns.hideColumn(tableId, { name: "a", type: "string" }), Columns.showColumn(tableId, { name: "a", type: "string" })]);
  ...
  test("show all columns", () => {
    store.dispatch(Columns.updateColumns(tableId, {
