...
test("getting the actual connection", () => {
  ...
  const state = store.dispatchAll([Clusters.add(conn), Current.setConnectionId("myconn")]);
...
test("getting the actual space", () => {
  ...
  const state = store.dispatchAll([Clusters.add(conn), Spaces.setDetail("myconn", space), Current.setConnectionId("myconn"), Current.setSpaceId(space.id)]);
