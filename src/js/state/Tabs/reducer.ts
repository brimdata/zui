...
export default function reducer(state: TabsState = init, action: TabActions) {
  ...
  switch (action.type) {
    case "TABS_ACTIVATE":
      if (state.data.map(t => t.id).includes(action.id)) return {
        ...state,
        active: action.id
      };else {
        return state;
...
function tabAction({
  ...
  return (type.startsWith("SEARCH_") || type.startsWith("VIEWER_") || type.startsWith("CHART_") || type.startsWith("COLUMNS_") || type.startsWith("HISTORY_") || type.startsWith("LOG_DETAIL_") || type.startsWith("LAYOUT_") || type.startsWith("CURRENT_") || type.startsWith("LAST_"));
}
