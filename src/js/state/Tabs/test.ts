...
test("add tab with data and activate", () => {
  let state = store.dispatchAll([Tabs.add("1", { connectionId: "a", spaceId: null }), Tabs.activate("1")]);
...
test("cannot activate tab that does not exist in data", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.activate("1"), Tabs.activate("does-not-exist")]);
  let tab = Tabs.getActiveTab(state);
...
test("remove last, active tab", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.activate("2"), Tabs.remove("2")]);
  expect(Tabs.getActive(state)).toBe("1");
});

test("remove middle, active tab", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.add("3"), Tabs.add("4"), Tabs.activate("2"), Tabs.remove("2")]);
  expect(Tabs.getActive(state)).toBe("3");
});

test("remove first, active tab", () => {
  let first = Tabs.getData(store.getState())[0].id;
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.add("3"), Tabs.add("3"), Tabs.remove(first)]);
  expect(Tabs.getActive(state)).toBe("1");
});

test("remove non-active tab before active tab", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.activate("2"), Tabs.remove("1")]);
  expect(Tabs.getActive(state)).toBe("2");
});

test("remove non-active tab after active tab", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.activate("1"), Tabs.remove("2")]);
  expect(Tabs.getActive(state)).toBe("1");
});
...
test("moving a tab to destination index", () => {
  let state = store.dispatchAll([Tabs.add("1"), Tabs.add("2"), Tabs.add("3"), Tabs.add("4"), Tabs.move("3", 0)]);
...
test("reorder tabs", () => {
  ...
  let state = store.dispatchAll([Tabs.add("a"), Tabs.add("b"), Tabs.add("c"), Tabs.order([3, 1, 2, 0])]);
...
test("reorder tabs does not throw error if invalid", () => {
  ...
  let state = store.dispatchAll([Tabs.add("a"), Tabs.add("b"), Tabs.add("c"), Tabs.order([0, 0, 0, 0])]);

  expect(Tabs.getData(state).map(t => t.id)).toEqual([first]);
});

test("reset tab", () => {
  let state = store.dispatchAll([Current.setSpaceId("myspaceid"), Tabs.clearActive()]);
