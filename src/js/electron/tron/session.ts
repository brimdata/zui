  ...
  return {
    ...
    load: async function (): Promise<SessionState | null | undefined> {
      ...
      if (await file.exists()) {
        return await file.read().then(JSON.parse).then(state => migrate(state, migrator)).then(state => state.data).catch(e => {
...
function ensureVersioned(state) {
  if (isNumber(state.version)) return state;else return {
    version: 0,
    data: state
  };
}
