...
export default function windowManager() {
  ...
  return {
    init(session: SessionState | null | undefined) {
      if (!session || (session && session.order.length === 0)) return this.openWindow("search");
    ...
    isQuitting(val: boolean | null | undefined) {
      if (isBoolean(val)) isQuitting = val;else return isQuitting;
    },
    ...
    fetchWindowStates() {
      return Promise.all(Object.keys(windows).map(id => {
    ...
    openSearchTab(searchParams: NewTabSearchParams) {
      let isNewWin = true;
      const existingWin = this.getWindows().sort((a, b) => a.lastFocused - b.lastFocused).find(w => w.name === "search");
      if (existingWin) {
        isNewWin = false;
        sendTo(existingWin.ref.webContents, ipc.windows.newSearchTab({ ...searchParams, isNewWin }));
      ...
      win.ref.webContents.once("did-finish-load", () => {
        sendTo(win.ref.webContents, ipc.windows.newSearchTab({ ...searchParams, isNewWin }));
      });
    },
    ...
    openWindow(name: WindowName, winParams: $Shape<WindowParams> = {}) {
      ...
      let ref = tron.window(name, params).on("focus", () => {
    ...
    openPreferences() {
      let win = this.getWindows().sort((a, b) => (b.lastFocused || 0) - (a.lastFocused || 0)).find(w => w.name === "search");
