...
export default function appMenu(send: Function, manager: $WindowManager, store: any, session: Session, platform: string = process.platform) {
  ...
  const resetState = {
    ...
    click: () => {
      send("resetState");
      lib.file(config.windowStateFile()).remove().catch(() => {});
    }
  };
  ...
  const brimMenu = {
    label: "Brim",
    submenu: [aboutBrim, __, preferences, { role: "services", submenu: [] }, __, { role: "hide" }, { role: "hideothers" }, { role: "unhide" }, __, { role: "quit" }]
  };
  ...
  function fileSubmenu() {
    if (mac) {
      ...
      return [newWindow, __, exportResults, __, preferences, __, closeTab, closeWindow, exit];
    }
  }

  function editSubmenu() {
    let submenu = [{ role: "undo" }, { role: "redo" }, __, { role: "cut" }, { role: "copy" }, { role: "paste" }, { role: "pasteandmatchstyle" }, { role: "delete" }, { role: "selectall" }];
    if (mac) {
      submenu.push(__, {
        label: "Speech",
  ...
  function viewSubmenu() {
    return [{ role: "reload" }, { role: "forcereload" }, { role: "toggledevtools" }, __, { role: "resetzoom" }, { role: "zoomin" }, { role: "zoomout" }, __, {
  ...
  function helpSubmenu() {
    let submenu = [{
      ...
      click() {
        shell.openExternal("https://join.slack.com/t/brimsec/shared_invite/zt-cy34xoxg-hZiTKUT~1KdGjlaBIuUUdg");
      }
    }, {
  ...
  let template = [{ label: "File", submenu: fileSubmenu() }, { label: "Edit", submenu: editSubmenu() }, { label: "Query", submenu: querySubmenu() }, { label: "View", submenu: viewSubmenu() }, { role: "window", submenu: windowSubmenu() }, { role: "help", submenu: helpSubmenu() }];
