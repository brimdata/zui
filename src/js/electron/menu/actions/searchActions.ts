...
import { appendQueryCountBy, appendQueryExclude, appendQueryIn, appendQueryInclude, appendQueryNotIn, appendQuerySortBy } from "../../../flows/searchBar/actions";
  ...
  return {
    ...
    groupByDrillDown: action({
      ...
      listener(dispatch, program, log) {
        let newProgram = brim.program(program).drillDown(brim.log(log.tuple, log.descriptor)).string();
    ...
    toTime: action({
      ...
      listener(dispatch, fieldData) {
        let field = brim.field(fieldData);
        if (field.type === "time") {
          dispatch(tab.setTo(brim.time(field.toDate()).add(1, "ms").toTs()));
          dispatch(submitSearch());
        }
      }
    }),
