...
const autoUpdateLinux = async () => {
  ...
  try {
    ...
    if (!semver.valid(currentVersion)) throw "Invalid current version format: " + currentVersion;

    const resp = await got(url).json();

    const latestVersion = get(resp, "name", "");
    if (!semver.valid(latestVersion)) throw "Invalid latest version format: " + latestVersion;
...
export function setupAutoUpdater() {
  ...
  autoUpdater.on("update-downloaded", (event, releaseNotes, releaseName) => {
    const dialogOpts = {
      ...
      detail: "A new version of Brim has been downloaded. Restart the application to apply the update."
    };
