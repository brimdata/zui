...
export default function PacketsButton({
  ...
  useDebouncedEffect(() => setEnabled(!!conn && space.pcap_support), 100, [conn, space.pcap_support]);
  ...
  function getTitle() {
    ...
    if (conn) return "Open packets from this connection.";else return "No connection record found.";
  }

  return <div className="packets-button" title={getTitle()}>
      <Button icon={<Sharkfin />} disabled={!enabled} onClick={onClick} {...reactElementProps("pcapsButton")} />
