...
export default function LogDetailsWindow() {
  ...
  return <NavAnimation log={log} prev={isGoingBack}>
      <div className="log-detail-window">
        <PaneHeader>
          <Left>
            <HistoryButtons prevExists={prevExists} nextExists={nextExists} backFunc={() => dispatch(LogDetails.back())} forwardFunc={() => dispatch(LogDetails.forward())} />
          </Left>
          <Center>
        ...
        <div className="log-detail-container">
          <div className="log-detail-body-wrapper">
            <div className="log-detail-body">
              <FieldsPanel log={log} contextMenu={menu.detailFieldContextMenu} />
              {log.correlationId() && <UidPanel log={log} />}
              <ConnPanel log={log} contextMenu={menu.detailFieldContextMenu} />
              {log.getString("md5") && <Md5Panel log={log} contextMenu={menu.detailFieldContextMenu} />}
            </div>
          </div>
        </div>
      </div>
