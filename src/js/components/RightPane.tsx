...
import Pane, { PaneHeader, PaneTitle, Left, Right, Center, PaneBody } from "./Pane";
...
export default class RightPane extends React.Component<Props, S> {
  ...
  render() {
    ...
    return <Pane isOpen={isOpen} onDrag={this.onDrag} position="right" width={width} className="log-detail-pane">
        {currentLog && <PaneHeader>
            <Left>
              <HistoryButtons prevExists={prevExists} nextExists={nextExists} backFunc={() => this.props.dispatch(LogDetails.back())} forwardFunc={() => this.props.dispatch(LogDetails.forward())} />
            </Left>
            <Center className="log-detail-center">
              <PaneTitle>Log Details</PaneTitle>
              <ExpandWindow onClick={() => this.props.dispatch(openLogDetailsWindow())} className="panel-button" />
            </Center>
            <Right>
              <CloseButton className="panel-button close-button" onClick={() => this.props.dispatch(Layout.hideRightSidebar())} />
            </Right>
          </PaneHeader>}
        <PaneBody>
          <LogDetailsComponent />
...
export const XRightPane = connect<Props, {}, _, _, _, _>(stateToProps, dispatchToProps)(RightPane);