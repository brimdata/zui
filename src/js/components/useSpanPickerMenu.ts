...
export default function useSpanPickerMenu() {
  ...
  }, { click: () => setSpan(spanOfLast(24, "hours")), label: "Last 24 hours" }, { click: () => setSpan(spanOfLast(7, "days")), label: "Last 7 days" }, { click: () => setSpan(spanOfLast(30, "days")), label: "Last 30 days" }];
  ...
  let to = brim.time(max_time).add(1, "ms").toDate();
  let spaceSpan = [from, to];

  return [{ click: () => setSpan(spaceSpan), label: "Whole Space" }, {
