...
function NodeRow({
  ...
  const findingSpaceName = get(node, ["data", "finding", "search", "spaceName"], "");
  ...
  function renderWarning() {
    const findingSpaceId = get(node, ["data", "finding", "search", "spaceId"], "");
  ...
  return <div key={i} className={className}>
      <div className="filter-tree-parent" onClick={onNodeClick} onContextMenu={() => menu.open()} title={findingSpaceName}>
        <FilterNode filter={node.data.filter} />
        {renderWarning()}
      </div>
      <div className="filter-tree-children">
        {node.children.map((node, i) => <NodeRow node={node} i={i} key={i} />)}
      </div>
    </div>;
}
...
export default function FilterTree() {
  ...
  if (tree.getRoot().children.length === 0) return <EmptySection icon={<BookIcon />} message="As you search through your data, your history will appear here." />;

  return <div className="filter-tree">
      {tree.getRoot().children.map((node, i) => <NodeRow node={node} i={i} key={i} />)}
    </div>;
}
...
export function nodeIsPinned(pinnedFilters: string[], node: Node | null | undefined) {
...
function nodeIsActive(pinnedFilters: string[], previous: string, node: Node | null | undefined) {
  return (node && node.data.filter === previous && isEqual(pinnedFilters, getPinnedFilters(node)));
}