...
export default function TimeInput({
  ...
  let initValue = isString(timeArg) ? timeArg : brim.time(timeArg).format(SPAN_TIME_FMT);
  ...
  return <Form onSubmit={submit} className="time-picker-button-input">
      <input type="text" ref={el} value={value} onChange={onChange} onBlur={submit} />
      <div className="input-suggestions">
        {value.length > 2 && error && <p className="suggestion error">{error}</p>}
