...
export default function TimeSteppers({
  show,
  onUp,
  onDown,
  style
}: Props) {
  const enter = (anime, el) => anime.timeline({
    easing: "easeOutExpo",
    duration: 400
  }).add({
  ...
  const exit = (anime, el) => anime.timeline({ duration: 100, easing: "linear" }).add({
    targets: el.querySelector(".step-down"),
    opacity: 0
  }).add({
    targets: el.querySelector(".step-up"),
...
function StepUp(props) {
  return <div className="stepper step-up" {...props}>
      <CarrotUp />
    </div>;
}

function StepDown(props) {
  return <div className="stepper step-down" {...props}>
      <CarrotDown />
    </div>;
}