...
export default function TimeButton({
  ...
  function onUp(e) {
    ...
    onChange(brim.time(timeArg).add(1, unit).toTs());
  }

  function onDown(e) {
    e.stopPropagation();
    if (isString(timeArg)) return;
    onChange(brim.time(timeArg).subtract(1, unit).toTs());
  }
  ...
  return <div className={classNames("time-picker-button", {
    hovering: fuzzy.hovering
  })} onMouseLeave={fuzzy.mouseLeave} onClick={onClick}>
      <div className="hover-zone" />
      <TimeSteppers show={fuzzy.hovering} onUp={onUp} onDown={onDown} style={{ transform: `translate(${x}px, ${y}px)` }} />
      <MenuBarButton onFocus={() => setEditing(true)} icon={icon}>
        {isString(timeArg) ? brim.relTime(timeArg).format() : <TimeDisplay ts={timeArg} onMouseEnter={updatePosition} />}
      </MenuBarButton>
...
function ChangedDot({
  ...
  let enter = { scale: [0, 1] };
  return <Animate show={show} enter={enter}>
