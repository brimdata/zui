...
export default function PopMenuPointy({
  ...
  return <>
      <PopMenuWrapper onClose={menu.close} isOpen={menu.isOpen} setRef={menu.ref} style={menu.style}>
...
function PopMenuWrapper({
  ...
  return ReactDOM.createPortal(<div className="pop-menu-overlay" onClick={onClose}>
      <div className="pop-menu-wrapper" ref={setRef} style={style}>
        {children}
      </div>
    </div>, lib.doc.id("context-menu-root"));
}

function PopMenuList({
  template,
  ...props
}) {
  return <ul {...props}>
      {template.map((item, i) => <PopMenuListItem item={item} key={i} />)}
    </ul>;
}

function PopMenuListItem({
  item
}) {
  switch (item.type) {
    case "divider":
...
function PopMenuPointer({
  ...
  return <svg style={style} className="pop-menu-pointer" viewBox="0 0 100 50">
