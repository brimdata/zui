...
export default function Viewer(props: Props) {
  ...
  let scrollHooks = useConst(null, () => ScrollHooks.create(onScrollStart, onScrollStop));
  ...
  return <div className="viewer" ref={props.innerRef} tabIndex="0">
      <Header columns={props.tableColumns} dimens={props.dimens} scrollLeft={scrollLeft} {...reactElementProps("viewer_header")} />
      <div className="view" onScroll={onScroll} style={{
...
const List = React.memo(function List(props) {
  return <div className="list" style={Styler.list(props.dimens)} {...reactElementProps("viewer_results")}>
      {props.chunks.map(chunk => <Chunk columns={props.tableColumns} logs={props.logs} rows={props.chunker.rows(chunk)} key={chunk} rowRenderer={props.renderRow} dimens={props.dimens} />)}
      {props.renderEnd()}
    </div>;
});