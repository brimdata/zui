...
export default function ErrorNotice() {
  ...
  return <NoticeBanner show={visible}>
...
function getComponent(error: BrimError) {
  if (error.type === "NetworkError") return NetworkErrorNotice;else if (error) return Default;else return None;
}
...
function Default({
  ...
  return <>
      ...
      {details && details.length > 0 && <div className="error-details">
          {details.flatMap(s => s.split("\n")).map((string, i) => <p key={i}>{string}</p>)}
        </div>}
    </>;
}