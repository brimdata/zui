...
export default function UidPanel({
  log
}: {log: Log;}) {
  let status = useSelector(LogDetails.getUidStatus);
  let logs = useSelector(LogDetails.getUidLogs).map(brim.record).map(brim.interop.recordToLog);

  return <div className="correlated-logs-panel detail-panel" {...reactElementProps("correlationPanel")}>
      <PanelHeading status={status}>Uid Correlation</PanelHeading>
      {logs.length === 0 && <InlineTableLoading title="Loading timeline..." rows={3} />}
