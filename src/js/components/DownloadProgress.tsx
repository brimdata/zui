...
export default class DownloadProgress extends React.Component<Props> {
  ...
  render() {
    ...
    return <div className={classNames("download-progress", {
      complete,
      error: error
    })}>
        ...
        <div className="progress-bar">
          <div className="progress-value" style={{ width: percentComplete * 100 + "%" }} />
        </div>
      </div>;
  }
}
...
const message = ({
  complete,
  error
}) => error ? `Download error: ${error}` : complete ? "Download Complete" : "Downloading";
...
export const XDownloadProgress = connect<Props, {}, _, {dispatch: Dispatch;}, _, _>(stateToProps)(DownloadProgress);