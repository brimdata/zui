...
export default function Pins() {
  ...
  function renderFilter(filter: string, index: number) {
    return <FilterNode key={index} filter={filter} focused={editing === index} pending={index === -1} onClick={() => {
  ...
  function renderPinButton() {
    return <div className="pin-button-wrapper">
        <span onClick={() => dispatch(SearchBar.editSearchBarPin(null))}>
          {fmtProgram(prevProgram)}
        </span>
        <button className="pin-button" title="⌘K" onClick={() => dispatch(SearchBar.pinSearchBar())}>
          <PinIcon />
        </button>
      </div>;
  }
