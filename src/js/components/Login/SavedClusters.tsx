...
export default function SavedClusters({
  saved,
  onClick,
  onRightClick
}: Props) {
  return <div className="saved-creds">
      <h3 className="section-heading">Saved Connections</h3>
      <ul>
        {saved.map(creds => <li key={creds.host + creds.port + creds.username} onClick={() => onClick(creds)} onContextMenu={() => onRightClick(creds)}>
