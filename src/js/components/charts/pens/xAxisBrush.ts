...
export default function (props: Props = {}): Pen {
  ...
  function draw(chart) {
    ...
    brushG.attr("transform", `translate(${chart.margins.left}, ${chart.margins.top})`);
    const brush = d3.brushX().extent([[0, 0], [innerWidth(chart.width, chart.margins), innerHeight(chart.height, chart.margins)]]);

    brushG.call(brush);
    chart.state.selection ? brush.move(brushG, chart.state.selection.map(chart.xScale)) : brush.move(brushG, null);
