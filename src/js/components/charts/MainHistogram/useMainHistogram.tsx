...
export default function (width: number, height: number): HistogramChart {
  ...
  let pens = useConst<Pen[]>([], () => {
    ...
    return [xAxisTime({ onDragEnd }), barStacks(), yAxisSingleTick(), xAxisBrush({ onSelection, onSelectionClear, onSelectionClick }), hoverLine(), reactComponent(chart => <EmptyMessage show={!chart.state.isFetching && chart.state.isEmpty} />), reactComponent(chart => <LoadingMessage show={chart.state.isFetching} message="Chart Loading" />), focusBar({ onFocus }), xPositionTooltip({
  ...
  return useMemo<HistogramChart>(() => {
    ...
    return {
      ...
      yScale: d3.scaleLinear().range([innerHeight(height, margins), 0]).domain([0, maxY]),
      xScale: d3.scaleUtc().range([0, innerWidth(width, margins)]).domain(span),
      pens
    };
