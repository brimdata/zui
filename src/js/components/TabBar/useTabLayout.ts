...
const idle = (springOrder: number[], width: number, first: boolean) => (springIndex: number) => {
...
const dragging = (springOrder: number[], dragSpringIndex: number, dragIndex: number, width: number, dx: number) => (springIndex: number) => {
...
function orderSprings(orderedIds: string[], map: Map<string, number>): number[] {
  // $FlowFixMe
...
export default function (tabs: TabState[], width: number) {
  ...
  // the new order.
  let [springs, set] = useSprings(ids.current.length, idle(springOrder, width, first));
