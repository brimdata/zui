...
export default function TabBar() {
  ...
  const calcWidth = () => setWidth(lib.bounded(rect.width / count, [0, MAX_WIDTH]));
  ...
  return <div className="tab-bar">
      <div className="tabs-container" ref={ref} onMouseLeave={ctl.onMouseLeave}>
        {tabs.map(tab => <AnimatedSearchTab {...layout.dragBinding({
        id: tab.id,
        onDown: () => ctl.onTabClick(tab.id),
        onChange: indices => ctl.onTabMove(indices)
      })} key={tab.id} title={brim.tab(tab, spaces).title()} style={layout.getStyle(tab.id)} removeTab={e => ctl.onRemoveClick(e, tab.id)} active={tab.id === ctl.activeId} isNew={false} />)}
