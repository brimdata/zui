...
export default function Animate({
  ...
  useLayoutEffect(() => {
    switch (state) {
      case "out":
        if (show) setState("entering");else seekToEnd(exitAni.current);
        break;
      case "entering":
        if (show) playEnter();else interuptEnter();
        break;
      case "in":
        if (show) seekToEnd(enterAni.current);else setState("exiting");
        break;
      case "exiting":
        if (show) interuptExit();else playExit();
        break;
  ...
  function playExit() {
    if (el.current) {
      exitAni.current = exit === "reverse" ? lib.animation(el.current, enter).reverse() : lib.animation(el.current, exit);
  ...
  if (state === "out") return null;else return React.cloneElement(React.Children.only(children), { ref: el });
}