...
export default function CurlModalBox() {
  ...
  return <ModalBox buttons={buttons} name="curl" className="curl-modal" title="curl command" {...reactElementProps("curlModal")}>
      <CurlModalContents />
    </ModalBox>;
}
...
function CurlModalContents() {
  ...
  const {
    ...
    host,
    program
  } = dispatch(inspectSearch());
  return <TextContent>
      {search && <pre id="copy-to-curl-code" {...reactElementProps("curlCommand")}>
