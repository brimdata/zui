...
* Use URLs to keep track of routing and tab history, which allows for direct edits of pinned (#1473, #1649)
...
* Brim now invokes [Brimcap](https://github.com/brimdata/brimcap) to generate logs from imported pcaps and to extract flows when **Packets** is clicked, rather than relying on `zqd` (#1584, #1573, #1591, #1590, #1598, #1614, #1617, #1637, #1651, #1664, #1668, #1705, #1731, #1735)
* Use Pools in Zed Lakes for backend storage rather than Spaces (#1589, #1602, #1631, #1676, #1696, #1710, #1712)
...
* Offer the option to migrate Spaces to Pools when new Brim launches (see docs)[https://github.com/brimdata/brim/wiki/Migration-of-Spaces-%28v0.25.0-%29)) (#1621, #1587, #1636, #1641, #1640, #1657, #1671, #1682)
* Automatically append a `| fuse` to the Zed pipeline if the user is exporting data in CSV format (#1622)
* Notify a Windows user if they've launched new Brim while old Brim is still installed (#1627)
* Update default **Query Library** entries to reflect newer Zed language syntax (#1645)
...
* Show Brim Release Notes for the currently-installed version in a tab (#1655, #1670, #1679, #1680)
...
* Update the [Brim wiki](https://github.com/brimdata/brim/wiki) articles for the `v0.25.0` release, including separate articles for old & new Brim where functionality has changed significantly (#1723)
* Update the [Code Base Walkthrough wiki article](https://github.com/brimdata/brim/wiki/Code-Base-Walkthrough) to reflect recent changes for where code lives in the repo (#1738)
