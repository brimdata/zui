...
async function zqArtifactsDownload(version, destPath) {
  ...
  try {
    ...
    for (let f of [plat.zqdBin, plat.zqBin]) {
      fs.moveSync(path.join(tmpdir.name, paths.internalTopDir, f), path.join(destPath, f), { overwrite: true });
    }
  } finally {
...
async function main() {
  try {
    ...
    if (zqdVersion.startsWith("v")) {
      ...
      let realZqVersion = child_process.execSync(path.join(zdepsPath, "zq") + " -version").toString().trim();
