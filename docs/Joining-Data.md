...
sources such as threat intelligence, domain/IP details, and so forth. A common
way to leverage such multiple data sources is to "join" them together.

The Z language includes a `join` processor that can be used for this. The
processor is still new/experimental and represents the first of what we expect
to be multiple ways of combining different data sources. This cookbook
describes an introductory `join` use case while describing its limitations.
...
data. However, we recognzie the approach is not as easy-to-use or as performant
as in-memory approaches that tools often take for smaller-scale joins. We hope
that user feedback will help us confirm the priority of which other approaches
to join we should offer in future releases.
...
bring multiple diverse data sources together into the same Space. While these
...
provides data on paylods in SSL-encrypted connections. The records produced by
the package include a `uid` value that could be joined to other well-known
Zeek record types that also include the `uid` such as `ssl` or `conn, allowing
the SPL-SPT data to appear as additional columns on these records as an
alternative to pivoting between the separate logs streams.
...
to generate logs from our example pcap. Here we've started from an basic
install on Linux via [binary packages](https://github.com/zeek/zeek/wiki/Binary-Packages).
After also installing the
...
explained that the tool operates on SSL connections. Looking inside the
`ssl.log` and `spl.log`, we can indeed see visually that the same `uid` values
appear in each

After having downloaded our [test wrccdc pcap](https://archive.wrccdc.org/pcaps/2018/wrccdc.2018-03-23.010014000000000.pcap.gz)
we run it through Zeek to generate logs.
...
as the "join-ready" data source. 
For this cookbook, we've chosen a join-ready data source that will hopefully
seem kk
