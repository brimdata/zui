...
[`zqd`](https://github.com/brimdata/zed/tree/v0.29.0/ppl/cmd/zqd) process uses an
...
currently "silent" on macOS and Linux.
...
found in the `zdeps` directory under the Brim [application binaries](https://github.com/brimdata/brim/wiki/Filesystem-Paths#application-binaries-v0240)
path.
...
to the [`zqd`](https://github.com/brimdata/zed/tree/v0.29.0/ppl/cmd/zqd) process that
...
If you run through the above steps using a pcap from which you've successfully
...
Operations in `zqd` are invoked via a [REST API](https://en.wikipedia.org/wiki/Representational_state_transfer)
...
engine for querying your data). These are located in the
