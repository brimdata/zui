...
export const runSearch = async (app: Application, searchText: string) => {
  ...
  await logStep("write to main search", () => app.client.waitForVisible(selectors.search.input).then(() => app.client.setValue(selectors.search.input, searchText)));
  await logStep("click the search button", () => app.client.keys("Enter"));
  return waitForResults(app);
};

export const getResults = async (app: Application, includeHeaders: boolean = true): Promise<string[]> => {
  ...
  const searchDisplayHeaders = async (app: Application): Promise<string[]> => {
    ...
    let headers = await logStep("get search headers", () => app.client.waitForVisible(selectors.viewer.headers).then(() => app.client.getHTML(selectors.viewer.headers)));
  ...
  let headers;
  if (includeHeaders && (await app.client.isVisible(selectors.viewer.headers))) {
