...
export const handleError = async (app: Application, initialError: Error, done: any) => {
  ...
  if (notificationError) {
    realError = new Error("App notification '" + notificationError + "' (initial error: '" + initialError + "'");
  } else {
...
export const stdTest = (descr: string, f: (done: any) => void | Promise<void>, timeout: number = TestTimeout) => {
  test(descr, done => {
