...
export default ((name: string) => {
  ...
  return {
    ...
    mockSaveDialog(value: {canceled: boolean;filePath: string | null | undefined;}) {
      return app.mainProcess.emit("spectron:mock", "windows:showSaveDialog", value);
    },

    setValue(locator: Locator, value: string) {
      return logStep(`set input ${locator.css} to ${value}`, () => app.client.waitForVisible(locator.css).then(() => app.client.setValue(locator.css, value)));
    },
    ...
    waitForText(locator: string, regex: RegExp) {
      return retryUntil(() => app.client.getText(locator), s => regex.test(s));
    },
