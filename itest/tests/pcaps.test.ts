...
describe("Test PCAPs", () => {
  ...
  stdTest("pcap button downloads deterministically-formed pcap file", done => {
    runSearch(app, "_path=ssl id.orig_h=192.168.1.110 id.resp_h=209.216.230.240 id.resp_p=443").then(async () => {
      ...
      expect(md5(readFileSync(pcapAbspath))).toBe("888453c81738fd8ade4c7f9888d86f86");
      done();
    }).catch(err => {
  ...
  stdTest("pcap download works for null duration", done => {
    runSearch(app, "duration=null id.orig_p=47783").then(async () => {
      await appStep.click(app, selectors.viewer.resultCellContaining("conn"));
      let downloadText = await appStep.savePcap(app);
      expect(downloadText).toBe("Download Complete");
      const fileBasename = "packets-1582646589.440467.pcap";
      let pcapAbspath = path.join((await pcapsDir(app)), fileBasename);
      expect(md5(readFileSync(pcapAbspath))).toBe("678442857027fdc5ad1e3418614dcdb8");
      done();
    }).catch(err => {
      handleError(app, err, done);
    });
