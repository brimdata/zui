...
const UNSET = "⦻";
const STRINGS = ["mystr", "\\x2d", "\\x09", '"', "'", ",", ";", "∫£œßü™", '"mystr"', "'mystr'", "*", '"*"', "1.1.1.1", " ", "null", "⦻" // The actual character, not unset.
];
...
describe("type-wise Filter = value searches", () => {
  ...
  stdTest("FilterEq ensure ingest", done => {
    appStep.search(app, `* | count() by _path | sort _path`).then(results => {
    ...
    FIELDS.forEach(fieldName => {
      values.forEach(s => {
        ...
        stdTest(`FilterEq${testId}: ${path} ${fieldName}="${s}"`, done => {
          runSearch(app, `_path=${path} ${fieldName}!=null | cut id, ${fieldName}`).then(async () => {
            await appStep.rightClick(app, selectors.viewer.resultCellContaining(s));
            await appStep.click(app, selectors.viewer.contextMenuItem("Filter = value"));
  ...
  stdTest(`FilterEq unset/${UNSET} string`, done => {
    runSearch(app, `_path=string | cut id, scalar | sort -r id | head 10`).then(async () => {
      await appStep.rightClick(app, selectors.viewer.resultCellContaining(UNSET));
      await appStep.click(app, selectors.viewer.contextMenuItem("Filter = value"));
      expect((await getResults(app))).toMatchSnapshot();
      done();
    }).catch(err => {
