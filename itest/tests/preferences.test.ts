...
import { defaultModalButton, jsonTypeConfigInput } from "../../src/js/test/locators";
...
describe("Preferences Modal Tests", () => {
  ...
  stdTest("setting a custom json type config", async done => {
    ...
    await brim.search("_path=conn proto=tcp | cut ts, src_ip, src_port, dst_ip, dst_port, proto | sort ts | head 10");
