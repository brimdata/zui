...
describe("doc urls", () => {
  ...
  const request = (path, url) => new Promise((good, bad) => {
    https // $FlowFixMe
    .request(url, { method: "HEAD" }, res => {
      if (res.statusCode === 200) good();else bad(`${res.statusCode}: ${path}${url}`);
    }).end();
  });

  Object.keys(ZeekLogDescriptions).map(stripSuffix).map(buildUrl).forEach(([path, url]) => {
    test(`doc url responds 200 for ${path}`, () => retry(() => request(path, url), 5));
  });
});