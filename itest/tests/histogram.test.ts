...
const verifySingleRectAttr = (app, pathClass, attr) => // We needn't wait on this selector: the stack has verifyPathClassRect()
...
app.client.getAttribute(`.${pathClass} rect`, attr).then(vals => {
  ...
  if (!Array.isArray(vals)) {
    throw new Error(`expected Array for ${pathClass} attr ${attr}; got ${vals}`);
  }
  vals.forEach(val => {
    expect(Number(val)).toBeGreaterThanOrEqual(dataSets.sample.histogram.rectAttrMin);
    expect(Number(val)).toBeLessThan(dataSets.sample.histogram.rectAttrMax);
  });
  return vals;
});

const verifyPathClassRect = (app, pathClass) => Promise.all(["x", "y", "width", "height"].map(attr => verifySingleRectAttr(app, pathClass, attr)));
...
describe("Histogram tests", () => {
  ...
  stdTest("histogram deep inspection", done => {
    ...
    LOG.debug("Pre-login");
    appStep.ingestFile(app, "sample.pcap").then(async () => {
      LOG.debug("Checking a histogram appears");
      // Verify that a histogram of at least *partial data* is
      // present.
      await retryUntil(() => app.client.$$(selectors.histogram.rectElem), rectElements => rectElements.length > 0).catch(() => {
      ...
      let pathClasses = await retryUntil(() => app.client.getAttribute(selectors.histogram.gElem, "class"), pathClasses => pathClasses.length === dataSets.sample.histogram.wholeSpaceDistinctPaths);
      ...
      let allRectValues = await Promise.all(pathClasses.map(async pathClass => await verifyPathClassRect(app, pathClass)));
      LOG.debug("Got all rect elements");
      expect(allRectValues.length).toBe( // Whereas we just counted g elements before, this breaks down rect
      ...
      allRectValues.forEach(pathClass => {
        ...
        pathClass.forEach(attr => {
          expect(attr.length).toBe(dataSets.sample.histogram.wholeSpaceRectsPerClass);
        });
      });
